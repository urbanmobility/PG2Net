...
class TrajPreSimple(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        self.loc_size = parameters.loc_size #14001
        self.loc_emb_size = parameters.loc_emb_size #500
        self.tim_size = parameters.tim_size #48
        self.tim_emb_size = parameters.tim_emb_size #10
        self.hidden_size = parameters.hidden_size#500
        self.use_cuda = parameters.use_cuda#True
        self.rnn_type = parameters.rnn_type#LSTM

        self.emb_loc = nn.Embedding(self.loc_size, self.loc_emb_size) #(14001,500)
        self.emb_tim = nn.Embedding(self.tim_size, self.tim_emb_size) #(48,10)

        input_size = self.loc_emb_size + self.tim_emb_size #(510)

        if self.rnn_type == 'GRU':
            self.rnn = nn.GRU(input_size, self.hidden_size, 1)
        elif self.rnn_type == 'LSTM':
            self.rnn = nn.LSTM(input_size, self.hidden_size, 1) #(550,500,1) (input_size,hidden_size,num_layers)
    ...
    def forward(self, loc, tim):
        h1 = Variable(torch.zeros(1, 1, self.hidden_size)) #[1,1,500]
        c1 = Variable(torch.zeros(1, 1, self.hidden_size)) #[1,1,500]
        if self.use_cuda:
            h1 = h1.cuda()
            c1 = c1.cuda()

        loc_emb = self.emb_loc(loc) #[10, 1, 500]
        tim_emb = self.emb_tim(tim)#[10,1,10]
        x = torch.cat((loc_emb, tim_emb), 2) #[10,1,510]
        x = self.dropout(x)
        #print("x",x.shape) [83, 1, 510]
        if self.rnn_type == 'GRU' or self.rnn_type == 'RNN':
            out, h1 = self.rnn(x, h1)
        elif self.rnn_type == 'LSTM':
            out, (h1, c1) = self.rnn(x, (h1, c1))
        # print("out",out.shape) [83, 1, 500]
        out = out.squeeze(1) #[10, 1, 500]
        out = F.selu(out) #[10,500]
        out = self.dropout(out)

        y = self.fc(out)#做分类  [10, 14001])
        score = F.log_softmax(y)  # calculate loss by NLLoss [10, 14001]
        return score
...
class TrajPreAttnAvgLongUser(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        self.tim_size = parameters.tim_size #48
        self.tim_emb_size = parameters.tim_emb_size#10
        self.uid_size = parameters.uid_size #937
        self.uid_emb_size = parameters.uid_emb_size#40
        self.hidden_size = parameters.hidden_size#500
        self.attn_type = parameters.attn_type#dot
        self.rnn_type = parameters.rnn_type#LSTM
        self.use_cuda = parameters.use_cuda#True

        self.emb_loc = nn.Embedding(self.loc_size, self.loc_emb_size)#14001,500
        self.emb_tim = nn.Embedding(self.tim_size, self.tim_emb_size)#48,10
        self.emb_uid = nn.Embedding(self.uid_size, self.uid_emb_size)#937,40

        input_size = self.loc_emb_size + self.tim_emb_size #510
        self.attn = Attn(self.attn_type, self.hidden_size) #(dot,500)
        self.fc_attn = nn.Linear(input_size, self.hidden_size) #(510,500)
    ...
    def forward(self, loc, tim, history_loc, history_tim, history_count, uid, target_len):
        ...
        loc_emb = self.emb_loc(loc) #(21,1,500)
        tim_emb = self.emb_tim(tim) #(21,1,10)
        x = torch.cat((loc_emb, tim_emb), 2) #(21,1,510)
        x = self.dropout(x)

        loc_emb_history = self.emb_loc(history_loc).squeeze(1) #[11,500]
        tim_emb_history = self.emb_tim(history_tim).squeeze(1) #[11,10]
        count = 0
        loc_emb_history2 = Variable(torch.zeros(len(history_count), loc_emb_history.size()[-1])).cuda() #[10, 500])
        tim_emb_history2 = Variable(torch.zeros(len(history_count), tim_emb_history.size()[-1])).cuda() #[10, 500])
        ...
        history = F.tanh(self.fc_attn(history))  #[10, 500]

        if self.rnn_type == 'GRU' or self.rnn_type == 'RNN':
            out_state, h1 = self.rnn(x, h1)
        elif self.rnn_type == 'LSTM':
            out_state, (h1, c1) = self.rnn(x, (h1, c1))
        out_state = out_state.squeeze(1) #[21,500]
...
class TrajPreLocalAttnLong(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        if self.rnn_type == 'GRU':
            ...
            self.rnn_encoder = nn.LSTM(input_size, self.hidden_size, 1)#(510,500)
            self.rnn_decoder = nn.LSTM(input_size, self.hidden_size, 1)#(510,500)
        elif self.rnn_type == 'RNN':
            self.rnn_encoder = nn.RNN(input_size, self.hidden_size, 1)
            self.rnn_decoder = nn.RNN(input_size, self.hidden_size, 1)

        self.fc_final = nn.Linear(2 * self.hidden_size, self.loc_size) #(1000,14001)
        self.dropout = nn.Dropout(p=parameters.dropout_p)
        self.init_weights()
    ...
    def forward(self, loc, tim, target_len):
        ...
        attn_weights = self.attn(hidden_state, hidden_history).unsqueeze(0) #获取注意力系数
        context = attn_weights.bmm(hidden_history.unsqueeze(0)).squeeze(0) #获得注意力
        # print("hidden_state", hidden_state.shape) [10, 500]
        #print("context",context.shape)[10, 500]
        out = torch.cat((hidden_state, context), 1)  # no need for fc_attn
        #print("out", out.shape) [10, 1000]
        out = self.dropout(out)
        y = self.fc_final(out)  #[10, 14001]
        score = F.log_softmax(y)
        return score
